...
// Add after other state variables near the top
const floatingNumbers = [];

// Add after the drawPlayer function
function addFloatingNumber(x, y, value, type = "damage") {
...
// Modify the drawPlayers function to include floating numbers
function drawPlayers(interpolation = 1) {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
...
// Add healing number when healed
socket.on("playerHealthUpdate", (data) => {
  const player = players[data.playerId];
  if (!player) return;
